...
for i in `ls *1302019*`; do cut -f2 $i > $i.o; done; #take the second column in tab file and make a new file with .o extension
paste `ls *.o` > all_counts.tsv #paste together (column after column) all these new files with the second column only
cut -f1 `ls *1302019* | head -1` > genenames.txt #take the first column of one of the tab files and make a gene names file
...
ls *1302019*.o > filenames.txt #make a txt file with all the filenames
...
awk '{print}' filenames.txt gene_counts.tsv > aihreseq_full_cutfilt_nodash_gene_counts_final.tsv #print out the filenames, followed by the gene counts in a row after row fashion
