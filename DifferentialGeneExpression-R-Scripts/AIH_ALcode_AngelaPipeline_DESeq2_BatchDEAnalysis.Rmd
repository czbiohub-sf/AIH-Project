...
Edit variables
```{r}
design = ~spl_plate+sex+age+fibrosis_groups
filename= paste("AIH_012919", "~spl_plate+sex+age+fibrosis_groups", "AIHnocirrhosis-vs-healthy", sep='_')
contrast=c("fibrosis_groups", "AIHnocirrhosis", "healthy")
```
...
metadata <- read.csv("../AIH-metadata/AIH_metadata_firstpass_ALedits_012919.csv", header=TRUE, row.names = 1)
metadata
...
subset(metadata_filt, fibrosis_groups == "AIHcirrhosis")
```
...
setwd("../DifferentialGeneExpression-Output/Amy_DESeq2_DEGenes_012919_updateFibrosisGroups/")
