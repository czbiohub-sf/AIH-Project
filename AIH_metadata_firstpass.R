...
AIH_metadata <- read.csv ("18_0925_AASLDAutoimmunePilot_DATA.csv", stringsAsFactors = FALSE)

#Filter for selected columns
AIH_metadata_firstpass <- dplyr::select(AIH_metadata, aasld_id, case_hl_du, du, sex, race, ethn, aih_type, te_coll, age_coll,  on_tx, igg_coll, ast_coll, alt_coll,  alp_coll, tbili_coll, response, relapse, decomp, lt, liver_death, age_hc, ast_hc, alt_hc, alkphos_hc, tbili_hc)
...
AIH_metadata_firstpass[9, ] <- AIH_metadata_firstpass [72,]
AIH_metadata_firstpass[19, ] <- AIH_metadata_firstpass [11,]
AIH_metadata_firstpass[29, ] <- AIH_metadata_firstpass [74,]
AIH_metadata_firstpass[39, ] <- AIH_metadata_firstpass [96,]
AIH_metadata_firstpass[49, ] <- AIH_metadata_firstpass [27,]
AIH_metadata_firstpass[59, ] <- AIH_metadata_firstpass [75,]
AIH_metadata_firstpass[69, ] <- AIH_metadata_firstpass [26,]
AIH_metadata_firstpass[79, ] <- AIH_metadata_firstpass [32,]
AIH_metadata_firstpass[89, ] <- AIH_metadata_firstpass [61,]
AIH_metadata_firstpass[99, ] <- AIH_metadata_firstpass [23,]


#Rename duplicate rows to the duplicate name
AIH_metadata_firstpass[9, 1] <- "AASLD-009"
AIH_metadata_firstpass[19, 1] <- "AASLD-019"
AIH_metadata_firstpass[29, 1] <- "AASLD-029"
AIH_metadata_firstpass[39, 1] <- "AASLD-039"
AIH_metadata_firstpass[49, 1] <- "AASLD-049"
AIH_metadata_firstpass[59, 1] <- "AASLD-059"
AIH_metadata_firstpass[69, 1] <- "AASLD-069"
AIH_metadata_firstpass[79, 1] <- "AASLD-079"
AIH_metadata_firstpass[89, 1] <- "AASLD-089"
AIH_metadata_firstpass[99, 1] <- "AASLD-099"
...
AIH_metadata_firstpass %<>% select (aasld_id, case_hl_du, sex, race, ethn, aih_type, te_coll, on_tx, igg_coll, response, relapse, decomp, lt, liver_death, age, ast, alt, bili, alkp, fib4, F03_F4)


#Make a column to indicate duplicated sample
AIH_metadata_firstpass$dup <- ifelse (grepl ("AASLD-009|AASLD-019|AASLD-029|AASLD-039|AASLD-049|AASLD-059|AASLD-069|AASLD-079|AASLD-089|AASLD-099", AIH_metadata_firstpass$aasld_id), print ("duplicate"), print ("case_cont"))
...
AIH_metadata_firstpass$case_hl_du <- ifelse (AIH_metadata_firstpass$case_hl_du == 1, print ("case"), 
                                             ifelse (AIH_metadata_firstpass$case_hl_du == 2, print ("control"), 
                                                  ifelse (is.na(AIH_metadata_firstpass$case_hl_du), print ("complex"), print ("NA"))
                                             ))
...
##Switch out complex cases into a different column
...
setwd("~/AIH-Project/AIH metadata/data")
write.csv (AIH_metadata_firstpass, file = "AIH_metadata_firstpass.csv", row.names = FALSE)


#Making factors from cont variable from sample colleciton (ast, alt, tbili, alkp, igg)

AIH_metadata_firstpass_cut <- read.csv ("AIH_metadata_firstpass.csv")
...
#Some summary statistics using dyplr to group and summarize data (mean, sd, iqr to assess break points)
...
ggplot(AIH_metadata_firstpass, aes (x = ast, fill = F03_F4)) +
...
ggplot(AIH_metadata_firstpass, aes (x = alt, fill = F03_F4)) +
