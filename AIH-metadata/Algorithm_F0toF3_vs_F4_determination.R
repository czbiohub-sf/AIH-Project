...
AIH_cases_QF4 <- read.csv(file = "AASLDAutoimmunePilot_DATA_2018-12-17_1525.csv", stringsAsFactors = FALSE)
...
overall_fibrosis_review <- read.csv ("AASLDAutoimmunePilot_DATA_2018-12-17_1525.csv")
...
ofr_reassign <- AIH_cases_QF4_filter %>% filter (str_detect(ofr, 'CL')) %>% mutate(ofr = str_sub(ofr, 12))
...
write.csv(AIH_cases_QF4_calls, file = "18_1219_AIH_cases_QF4_calls.csv", row.names = FALSE)
