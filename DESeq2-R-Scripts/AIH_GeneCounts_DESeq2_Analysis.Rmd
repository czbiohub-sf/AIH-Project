...
```{r}
log_genecountspc <- log(genecountspc)
head(vstassay)
?prcomp
genecountspc_pca <- prcomp(vstassay, center = TRUE,
                 scale. = TRUE) 

plot (genecountspc_pca)
...
write.table(resDiag, "AIH_Diagnosis_nofilter_DESeq2_Results.tsv", sep="\t")
```
...
write.csv(resDiag_subset, file="AIHvControl_sexinc_nofilter_FDR0.2_Genes.csv")
...
write.csv( as.data.frame(normcounts), file="NormCounts_sexinc_nofilter_AIH-v-healthy.csv" )
```
...
?plotPCA
data <- plotPCA(vst, intgroup=c("case_hl_du","sex", "race", "ethn"), returnData=TRUE)
