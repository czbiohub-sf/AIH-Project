...
There are two different ways to control for batch effect: you can account for the variation generated by batch in your model (design = ~ batch + condition), or you can remove the batch effect prior to analysis (limma::removeBatchEffect)

Let's start with DESeq2 controlling of batch effect.
...
dds <- DESeqDataSetFromMatrix(countData = genecountspc, colData = metadata_filt, design= ~ case_hl_du)
...
Estimate size factors and perform a variance stabalizing transformation (for WGCNA analysis)
```{r}
...
write.csv (t_matrix_vsd, file = "VSDNormalized_AIH_101518.csv")
```
...
plotMA (results_MA, ylim = c(-3, 3))

# Make a basic volcano plot
with(results_MA, plot(log2FoldChange, -log10(pvalue), pch=20, main="Volcano plot, ~condition", xlim=c(-3,3)))
