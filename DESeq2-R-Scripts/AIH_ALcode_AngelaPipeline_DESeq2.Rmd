...
#only look at cases with response data
metadata_filt <- metadata_filt[!(is.na(metadata_filt$response)), ]
...
resultsNames(dds)
results <- results(dds, contrast=list("complete",c("non-responder","partial")), alpha=0.05, tidy = TRUE)
?results
View(results)
```
...
setwd("~/Documents/Informatics/AIH/github/AIH-Project/AIH DESeq2 Output/")
write.csv(results, "AIH_case_v_control_nooutliers_nolivertrans.csv")

write.csv(results_significant, "AIH_case_v_control_nooutliers_nolivertrans_FDR0.05.csv")
```
```{r}
