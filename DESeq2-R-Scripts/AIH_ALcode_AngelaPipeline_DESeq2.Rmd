...
results_significant %>% count(results_significant$log2FoldChange>0)

setwd("../AIH-DESeq2-Output/Amy_DESeq2_DEGenes_110118/")
write.csv(results, "AIH_DESeq2_~0+case_hl_du_110118d_prefiltering.csv")
...
write.csv (t_matrix_vsd, file = "VSDNormalized_case_control_prefiltered-110118.csv")
```
...
with(results_MA, plot(log2FoldChange, -log10(pvalue), pch=20, main="Volcano plot, ~ 0+case, prefiltered", xlim=c(-3,3)))
...
ggplot(pca_dds, aes(PC1, PC2, color= case_hl_du)) +
  ...
  labs(title="~ 0+case, prefiltered", position = "center")+
