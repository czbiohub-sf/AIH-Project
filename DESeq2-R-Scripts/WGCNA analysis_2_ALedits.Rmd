...
```{r}
setwd("../AIH-DESeq2-Output/")

#Reading in expression data
t_matrix_vsd <- read.csv("VSDNormalized_case_control_nooutliers_nolivertransplants_101618.csv", row.names = 1)
...
WGCNA_GO_analysis = paste("EdgeR_LocusLinkIDs-", module, ".txt", sep="")
#for DESeq2: table = cbind(as.data.frame(modLLIDs),results[results$row %in% modLLIDs,])
table = cbind(as.data.frame(modLLIDs),allgenes[rownames(allgenes) %in% modLLIDs,])
